...
外卖模式
1 外卖模式地址选择点击范围扩大至整个地址信息(陈波) ok
2 新增地址保存之后刷新列表(陈波)  ok
3 已经选择地址 进入下单页面不默认选择第一个(陈波)   ok
4 点餐包含配送费折扣不对无法支付的问题(陈波)    ok
5 后付款加菜确认下单点击多次存在重复加菜的情况(陈波)ok
6 后付款下单稍后再付下单页显示空白的问题(陈波) ok
6 查看详情红包金额显示问题(泳贤)  ok
7 配送费自动添加(泳贤)  ok 
8 后付款支持选择人数以及自动添加必选菜(泳贤)  ok
9 点餐完成支付后支付完成页面点击返回直接返回点餐页面优化(泳贤) ok
  暂时只支持头部的返回箭头点击返回到点餐页面,手动滑动的回退页面暂时不做支持修改默认的页面栈可能出错后续再优化
